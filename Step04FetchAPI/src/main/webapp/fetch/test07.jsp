...
<body>
	...
	<script>
		document.querySelector("#myForm2").addEventListener("submit", (e)=>{
			e.preventDefault();
			//my_util.js안에있는 ajaxFormPromise()함수 활용하기
	...
	/*
		폼안에 있는 submit버튼을 누르면 폼요소에는 "submit"이벤트가 발생한다
		따라서 "submit"이벤트가 발생했을 떄 원하는 동작이 있다면
		.addEventListener("submit", ()=>{})을 활용하면 된다
	*/
	document.querySelector("#myForm").addEventListener("submit", (e)=>{
		//event객체 안에 있는 preventDefault()함수를 호출하면
		e.preventDefault();
		//fetch를 이용해서 페이지 전환없이 입력한 내용을 읽어와서 직접 제출해야한다 
		
		//입력한 내용을 query문자열로 읽어오기
